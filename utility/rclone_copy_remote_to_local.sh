...
if [ "$#" -ne 2 ]; then
    ...
    echo "Esempio: $0 GDrive-F4Frank: /home/frankel/GDrive-F4Frank"
    exit 1
fi

# 2. Controllo dell'esistenza della directory locale
if [ ! -d "$DEST_LOCAL_PATH" ]; then
    echo "Errore: La directory locale '$DEST_LOCAL_PATH' non esiste."
    exit 1
fi
...
( # Lock BLOKKANTE generale per il remote (FD 201)
    ...
    ( # Lock NON-BLOKKANTE specifico per questo script (FD 200)
        ...
        /usr/bin/rclone copy "$SRC_REMOTE_PATH" "$DEST_LOCAL_PATH" \
        --max-delete 20 \
        --fast-list \
